...
const books: { id: number; name: string; price: number; }[] = [
...
function showMenu(): void {
    ...
    console.log('3. Exit');
    scanner.question('Pilih menu : ', (choice) => {
        if (choice === '1') {
            listBooks();
        } else if (choice === '2') {
            showCart();
...
function listBooks(): void {
    ...
    console.log("===========================");
    books.forEach((book) => {
        console.log(`${book.id}. ${book.name}  Rp. ${book.price}`);
    });

    scanner.question('Pilih buku : ', (bookId) => {
        const book = books.find(b => b.id === parseInt(bookId));
        if (book) {
            scanner.question('Input qty: ', (qty) => {
...
function showCart(): void {
    ...
    if (cart.length === 0) {
        ...
        console.log('------------------------------------------------------------------------')
        cart.forEach((book, i) => {
            console.log(`${i + 1}. ${book.name} | qty = ${book.quantity}, | Harga = Rp. ${book.price * book.quantity}`);
...
function deleteItem(): void {
    ...
    scanner.question('Pilih Item yang ingin dihapus:', (itemNumber) => {
...
function editQuantity(): void {
    ...
    scanner.question('Pilih item yang ingin diedit: ', (itemNumber) => {
        if (!isNaN(parseInt(itemNumber)) && parseInt(itemNumber) > 0) {
            const itemIndex = parseInt(itemNumber) - 1;
            if (itemIndex >= 0 && itemIndex < cart.length) {
                scanner.question('Input quantity: ', (newQuantity) => {
